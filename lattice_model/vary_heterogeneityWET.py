...
if __name__ == "__main__":
    ...
    for input in inputs:
        sigma = input[0]
        lazy_result = dask.delayed(do_jobWET)(sigma)
        lazy_results.append(lazy_result)
    results = dask.compute(*lazy_results)


    if not os.path.exists("het_results_WET"):
        os.makedirs("het_results_WET")
    np.savetxt("het_results_WET/params_%d.txt"%int(sys.argv[1]),inputs)
    for i, result in enumerate(results):
        save_npz("het_results_WET/%d_%d.npz"%(i,int(sys.argv[1])),csc_matrix(result))
    #
    # etx = ETX()
    #
