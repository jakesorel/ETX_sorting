...
if __name__ == "__main__":
    ...
    sigma_space = np.logspace(0,2.5,10)
    ...
    for input in inputs:
        sigma = input[0]
        lazy_result = dask.delayed(do_jobWEE)(sigma)
        lazy_results.append(lazy_result)
    results = dask.compute(*lazy_results)


    if not os.path.exists("het_results"):
        os.makedirs("het_results")
    np.savetxt("het_results/params_%d.txt"%int(sys.argv[1]),inputs)
    for i, result in enumerate(results):
        save_npz("het_results/%d_%d.npz"%(i,int(sys.argv[1])),csc_matrix(result))
    #
    # etx = ETX()
    #
