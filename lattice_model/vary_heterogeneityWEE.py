...
if __name__ == "__main__":
    ...
    if not os.path.exists("het_results_WEE"):
        os.makedirs("het_results_WEE")
    np.savetxt("het_results_WEE/params_%d.txt"%int(sys.argv[1]),inputs)
    for i, result in enumerate(results):
        save_npz("het_results_WEE/%d_%d.npz"%(i,int(sys.argv[1])),csc_matrix(result))
    #
    # etx = ETX()
    #
